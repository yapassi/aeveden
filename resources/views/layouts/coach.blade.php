    ...
    <header class="navbar navbar-dark bg-dark sticky-top">
      <div class="container-fluid">
        ...
        <div class="d-flex align-items-center text-white">
          <strong>{{ ucfirst($user->role) }}  {{ $user->prenoms }} {{ $user->nom }}</strong>
        </div>
      </div>
    </header>
    ...
    <div class="d-flex flex-grow-1">
      ...
      <nav id="sidebar" class="bg-light flex-shrink-0 p-3 border-end">
        <ul class="nav nav-pills flex-column">
          <li class="nav-item">
            <a class="nav-link {{ request()->routeIs('coach.dashboard') ? 'active fw-bold' : '' }}" href="{{ route('coach.dashboard') }}">
              <i class="bi bi-house me-2"></i>Accueil
            </a>
          </li>
          <li class="nav-item mt-3">
            <a href="{{ route('coach.profile.edit') }}" class="nav-link {{ request()->routeIs('coach.profile.*') ? 'active fw-bold' : '' }}">
              <i class="bi bi-person-circle me-2"></i>Profil
            </a>
          </li>
          <li class="nav-item mt-3">
            <a href="{{ route('coach.fiances.index') }}" class="nav-link {{ request()->routeIs('coach.fiances.*') ? 'active fw-bold' : '' }}">
              <i class="bi bi-person-heart me-2"></i>Fiancés
            </a>
          </li>
          <li class="nav-item mt-3">
            <a href="{{ route('coach.fiancailles.index') }}" class="nav-link {{ request()->routeIs('coach.fiancailles.*') ? 'active fw-bold' : '' }}">
              <i class="bi bi-gem me-2"></i>Fiançailles
            </a>
          </li>
          <li class="nav-item mt-3">
            <a href="{{ route('coach.coachings.index') }}" class="nav-link {{ request()->routeIs('coach.coachings.*') ? 'active fw-bold' : '' }}">
              <i class="bi bi-people-fill me-2"></i>Coachings
            </a>
          </li>
          <li class="nav-item mt-3">
            <a href="{{ route('coach.rapports.index') }}" class="nav-link {{ request()->routeIs('coach.rapports.*') ? 'active fw-bold' : '' }}">
